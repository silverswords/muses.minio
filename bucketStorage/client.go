...
func (m *MinioClient) PresignedPutObject(ctx context.Context, bucketName string, objectName string, expires time.Duration) (*url.URL, error) {
	u, err := m.mc.PresignedPutObject(ctx, bucketName, objectName, expires)
	if err != nil {
		return nil, err
	}
